...
A SQLite database is a single file on the user's computer, and instead of being a separate process, it's a set of C functions
...
Migration CI and failed-migration remediation guidance is tracked separately.
...
SQLite in with the `diesel_cli`. We use this version of Diesel and SQLite instead of relying on
...
official website https://www.sqlite.org/download.html. We use SQL language features that
are not available on old versions of SQLite, and you'll have to approve the binary to run in macOS System Preferences.
